...
function buildDocHtml(title, bodyHtml, options = {}){
  const { extraCss = "", bodyClass = "", hideTitle = false } = options;
  const bodyCls = bodyClass ? ` class="${esc(bodyClass)}"` : "";
  const titleHtml = hideTitle ? "" : `<h1>${esc(title)}</h1>`;
 ...
 */
export async function printHtmlDocument(title, bodyHtml, options = {}){
  const html = buildDocHtml(title, bodyHtml, options);
...
export async function downloadHtmlDocument(filename, title, bodyHtml, options = {}){
  const html = buildDocHtml(title, bodyHtml, options);
